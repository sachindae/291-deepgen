...
def make_rnn_cell(rnn_layer_sizes,
    ...
    print(cell)
    if attn_length:
        cell = tf.compat.v1.nn.rnn_cell.AttentionCellWrapper(cell, attn_length, state_is_tuple=state_is_tuple, reuse=reuse)
...
class RNNGAN(object):
    ...
    def __init__(self, is_training, num_song_features=None, num_meta_features=None, conditioning='multi'):
        ...
        self.rnn_pretraining_loss = tf.reduce_mean(
            input_tensor=tf.math.squared_difference(x=tf.transpose(a=tf.stack(self._generated_features_pretraining), perm=[1, 0, 2]),
                                  y=self._input_songdata))
        if not DISABLE_L2_REG:
            self.rnn_pretraining_loss = self.rnn_pretraining_loss + reg_loss

        pretraining_grads, _ = tf.clip_by_global_norm(tf.gradients(ys=self.rnn_pretraining_loss, xs=self.g_params),
                                                      MAX_GRAD_NORM)
        ...
        if conditioning == 'none' or LOSS_WRONG_D is False:
            ...
            self.d_loss = tf.reduce_mean(input_tensor=-2 * tf.math.log(tf.clip_by_value(self.real_d, 1e-1000000, 1.0))
                                         - tf.math.log(1 - tf.clip_by_value(self.generated_d, 0.0, 1.0 - 1e-1000000))
                                         - 2 * tf.math.log(1 - tf.clip_by_value(self.wrong_d, 0.0, 1.0 - 1e-1000000)))
        self.g_loss_feature_matching = tf.reduce_sum(
            input_tensor=tf.math.squared_difference(self.real_d_features, self.generated_d_features))
        self.g_loss = tf.reduce_mean(input_tensor=tf.math.log(tf.clip_by_value(1-self.generated_d, 1e-1000000, 1.0)))
        ...
        if FEATURE_MATCHING:
            g_grads, _ = tf.clip_by_global_norm(tf.gradients(ys=self.g_loss_feature_matching,
                                                             xs=self.g_params), MAX_GRAD_NORM)
        else:
            g_grads, _ = tf.clip_by_global_norm(tf.gradients(ys=self.g_loss, xs=self.g_params), MAX_GRAD_NORM)
